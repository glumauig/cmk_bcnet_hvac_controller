...
def discover_hvac_controller_alarm_reset(section):
    yield Service()

def check_hvac_controller_alarm_reset(section):
    ...
    if hvac_snmp_value == 0:
        ...
        summary_hvac = "Alarm Reset: " + str(hvac_snmp_value_converted)
        yield Result(state=State.OK,summary=summary_hvac)

    else:
        hvac_snmp_value_converted = "Alarm"
        summary_hvac = "Alarm Reset: " + str(hvac_snmp_value_converted)
...
register.check_plugin(
    name = "hvac_controller_alarm_reset",
    service_name='HVAC Controller alarm_reset',
    discovery_function= discover_hvac_controller_alarm_reset,
    check_function= check_hvac_controller_alarm_reset,
    )
