...
def discover_hvac_controller_temp_flow_return(section):
    yield Service()

def check_hvac_controller_temp_flow_return(section):
    ...
    summary_hvac = "Temperature Flow Return " + ": "+str(hvac_snmp_value)+" F" +", "+str(hvac_snmp_value_converted)+" C"
...
register.snmp_section(
    name = "hvac_controller_temp_flow_return",
    detect = exists(".1.3.6.1.4.1.15255.1.2.1.3.1.5.*"),
    fetch = SNMPTree(
        base = '.1.3.6.1.4.1.15255.1.2.1.3.1',
        oids = [
                '5.10', #temp_flow_return
                '2.10', #temp_flow_return_Value
        ],
    ),
)
...
register.check_plugin(
    name = "hvac_controller_temp_flow_return",
    service_name='HVAC Controller Temperature Flow Return',
    discovery_function= discover_hvac_controller_temp_flow_return,
    check_function= check_hvac_controller_temp_flow_return,
    )
