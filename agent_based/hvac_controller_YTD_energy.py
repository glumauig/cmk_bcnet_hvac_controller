...
def discover_hvac_controller_YTD_energy(section):
    yield Service()

def check_hvac_controller_YTD_energy(section):
...
register.snmp_section(
    name = "hvac_controller_YTD_energy",
    detect = exists(".1.3.6.1.4.1.15255.1.2.1.3.1.5.*"),
    fetch = SNMPTree(
        base = '.1.3.6.1.4.1.15255.1.2.1.3.1',
        oids = [
                '5.2', #YTD_Energy_Total
                '2.2', #YTD_Energy_Total_Value
        ],
    ),
)
...
register.check_plugin(
    name = "hvac_controller_YTD_energy",
    service_name='HVAC Controller Year to Date Energy Total',
    discovery_function= discover_hvac_controller_YTD_energy,
    check_function= check_hvac_controller_YTD_energy,
    )
